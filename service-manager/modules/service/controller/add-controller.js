(function($) {
    ...
    ngmod.controller("serviceAddCtrl",function($scope,serviceRES){
        $scope.project ={};
        ...
        serviceRES.get({
            ...
            $scope.categoryList = project.models;
//            var flist = {"required":"非必须","position":"0"};
//            $scope.$broadcast("copyDataLoaded",{inputJson:flist,outputJson:flist,opMethod:"add"})
        });
        ...
        $scope.onInitialized = function(grid) {
            ...
            grid.selector.on('rowdoubleclick', function (event)
            {
                ...
                $scope.openModal({
                    title:"修改信息"
                    ,remote: $scope.MODULE_PATH +"service/add-page.shtml"
                    ,width:680
                    ,height:580
                    ,data:{
                        //传过去的数据
        ...
        $scope.plusOut = function(){
            var count =  $scope.grid2.selector.jqxGrid('getdatainformation').rowscount;
            $scope.openModal({
                title:"添加数据"
                ,remote: $scope.MODULE_PATH +"service/add-page.shtml"
                ,width:680
                ...
                ,onSubmit:function(modalConfig){
                    //这里加关闭窗口操作
                    $scope.grid2.selector.jqxGrid('addrow', null, modalConfig.resultData);
                    return true;//返回true，就自动关闭窗口，否则不关闭。可用于手工控制窗口的打开与关闭
                }
            });
        }
